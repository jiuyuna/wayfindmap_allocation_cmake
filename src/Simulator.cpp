...
void Simulator::Stagnate_Avoid()
{
    for (size_t i = 0; i < agents.size(); ++i)
    {
        ...
        //迷路
        if (cur_agent->record_time >= 50) //&& (!cur_agent->isKnown)
        {
            if (cur_agent->numOfTry <= 3)
                cur_agent->record_time = 0; //清除异常
            cur_agent->numOfTry++;
        ...
        else if (cur_agent->isFind && !cur_agent->isKnown)
        {
            ...
            for (int j = 0; j < n_signages; ++j)
            {
                ...
                if (dist <= signages[j].vca_r && !isObstacle(cur_agent->x, cur_agent->y, cur_agent->gx, cur_agent->gy)) // && theta <= cur_agent->FOV/2
                {
                    ...
                    cur_agent->temp_gx = cur_agent->gx;
                    cur_agent->temp_gy = cur_agent->gy;
...
void Simulator::SFM()
{
    for (size_t i = 0; i < agents.size(); ++i)
    {
        ...
        //记录位置变化微小的agent
        if (abs(cur_agent->last_x - cur_agent->x) + abs(cur_agent->last_y - cur_agent->y) < 1e-2 && isObstacle(cur_agent->x, cur_agent->y, cur_agent->gx, cur_agent->gy))
        {
